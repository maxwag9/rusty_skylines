...
//
// Expects bindings:
//   @group(0) @binding(0) depth_tex  : texture_depth_2d
//   @group(0) @binding(1) normal_tex : texture_2d<f32>
//   @group(1) @binding(0) Uniforms   : your existing camera uniforms
//
...
// If your normal buffer stores normals in [0..1], keep this decode.
// If it stores normals already in [-1..1], change to: normalize(n.xyz).
...
fn fs_main(@builtin(position) frag_pos: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    // We compare view-space Z. In typical right-handed view space, forward is -Z.
    // "Closer" means less negative (greater Z).
    for (var i: u32 = 0u; i < KERNEL_SIZE; i = i + 1u) {
        var sample_dir = tbn * normalize(KERNEL[i]);

        // concentrate samples nearer the origin
        let fi = f32(i) / f32(KERNEL_SIZE);
        let scale = mix(0.10, 1.0, fi * fi);
