...
impl RoadRenderSubsystem {
    ...
    pub fn update(
        ...
        for v in &terrain_renderer.visible {
            ...
            // 1) ensure CPU mesh is current (only rebuild if needed)
            ...
            if mesh.indices.is_empty() || mesh.vertices.is_empty() {
                // Optional: free GPU for empty chunks
                self.chunk_gpu.remove(&chunk_id);
                continue;
            }

            // 2) ensure GPU mesh is current (upload only if topo_version differs)
            ...
            if needs_gpu_upload {
                // simplest: recreate buffers on change (fine because it’s per-chunk, not per-frame)
...
impl RoadMeshRenderer {
    ...
    //         gpu.add_draw_call(draw_call); // same as push draw call
    //
