...
pub enum TerrainEdit {
    ...
    FlatPolygon {
        ...
        /// Non-None when generated by the road system.
        source: Option<RoadEditElement>,
    },
    ...
    FlatPolyline {
        ...
        /// Height at each centerline point; must be the same length as `centerline`.
        heights: Vec<f32>,
        half_width: f32,
        terrain_offset: f32,
        falloff_distance: f32,
...
impl TerrainEdit {
    ...
    pub fn source(&self) -> Option<RoadEditElement> {
        match self {
            Self::Raise { .. } => None,
            Self::FlatPolygon { source, .. }
            | Self::FlatPolyline { source, .. }
            | Self::FlatCircle { source, .. } => *source,
        }
    }
...
impl TerrainEditor {
    ...
    pub fn remove_edits_for_source(&mut self, element: RoadEditElement) -> HashSet<ChunkCoord> {
        let mut affected = HashSet::new();
        let mut i = 0;
        while i < self.edits.len() {
            if self.edits[i].source() == Some(element) {
    ...
    pub fn push_raise(&mut self, center: WorldPos, radius: f32, strength: f32) -> EditId {
    ...
    pub fn apply_segment_flattening(
        ...
        terrain_offset: f32,
        falloff_distance: f32,
        _chunks: &HashMap<ChunkCoord, ChunkMeshLod>,
    ) -> HashSet<ChunkCoord> {
        let source = RoadEditElement::Segment(segment_id);
        // Replace any previous edit for this segment.
        self.remove_edits_for_source(source);
        let id = self.alloc_id();
        let edit = TerrainEdit::FlatPolyline {
            id,
            centerline: centerline.to_vec(),
            heights: road_heights.to_vec(),
            half_width,
            terrain_offset,
            falloff_distance,
            source: Some(source),
    ...
    pub fn apply_intersection_flattening(
        ...
        let source = RoadEditElement::Intersection(node_id);
        self.remove_edits_for_source(source);
        let id = self.alloc_id();
    ...
    pub fn remove_segment_flattening(&mut self, segment_id: SegmentId) -> HashSet<ChunkCoord> {
        self.remove_edits_for_source(RoadEditElement::Segment(segment_id))
    }

    pub fn remove_intersection_flattening(&mut self, node_id: NodeId) -> HashSet<ChunkCoord> {
        self.remove_edits_for_source(RoadEditElement::Intersection(node_id))
    }

    // ── Save / load ───────────────────────────────────────────────────────────

    /// Returns the edit list to be serialised (e.g. stored inside a larger save struct).
...
fn apply_edit_to_grid(
    ...
    match edit {
        ...
        TerrainEdit::FlatCircle {
            ...
            let target = height + terrain_offset;
        ...
        TerrainEdit::FlatPolyline {
            ...
            if centerline.is_empty() || centerline.len() != heights.len() {
                return;
            }
            let total = half_width + falloff_distance;
            ...
            for gx in 0..grid.nx {
                for gz in 0..grid.nz {
                    ...
                    let wz = chunk_wz + gz as f64 * cell;
                    let (dist, road_h) =
                        closest_point_on_polyline_world(wx, wz, centerline, heights, cs);
                    if dist > total {
                        continue;
                    }
...
fn closest_point_on_polyline_world(
    wx: f64,
    wz: f64,
    polyline: &[WorldPos],
    heights: &[f32],
    ...
    for i in 0..polyline.len().saturating_sub(1) {
        ...
        if dist < best_dist {
            best_dist = dist;
            best_h = heights[i] + (heights[i + 1] - heights[i]) * t as f32;
        }
    }
