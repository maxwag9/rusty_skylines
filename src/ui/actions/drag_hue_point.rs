...
pub fn drag_hue_point(loader: &mut UiButtonLoader, mouse_state: &MouseState, time: &TimeSystem) {
    ...
    let dragging = loader.touch_manager.drag.active_drag.is_some();
    let just_selected = loader.touch_manager.selection.just_selected;
    let just_deselected = loader.touch_manager.selection.just_deselected;
