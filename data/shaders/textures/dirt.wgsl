...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let wp = p + (vec2<f32>(wx, wy) - 0.5) * (0.8 + params.roughness * 1.8);

    let clump = fbm(wp, T);
    let fine = vnoise(wp * 10.0, T * 10.0);
    let pits = vnoise(wp * 4.0, T * 4.0);

    let base = pow(clamp(clump, 0.0, 1.0), 1.15);
    let micro = smoothstep(0.25, 0.85, fine) * 0.16;
    let cavity = smoothstep(0.55, 0.95, 1.0 - pits) * 0.12;
    let shade = clamp(base * 0.72 + micro + cavity, 0.0, 1.0);

    var color = mix(params.color_primary.rgb, params.color_secondary.rgb, shade);
    color *= 0.86 + base * 0.14;
